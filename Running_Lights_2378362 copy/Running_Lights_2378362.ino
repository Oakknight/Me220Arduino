...
void loop() {
  if(!modeChosen){
  if(Serial.available() > 0 ){ //no need to run if there is no input
  incomingMode = Serial.read(); //Read user input
  switch (incomingMode) { //switch to choose the working mode
  case 49:
...
void RunningLightsPause(){ //Vanilla with pause, TODO: Fix
...
void TimerCountUp(){ 
  ...
  if(timeCounter <= 255) {  // if we have less or equal to 255 presses
...
void ButtonCountUp(){
  attachInterrupt(digitalPinToInterrupt(interruptPin), AddCount, RISING);
}
...
void BinaryDisplay(){
  ...
  /* get the length of the string */
  int binLength = binNumber.length(); 
  if(userInput <= 255) {  // if we have less or equal to 255 presses
                // here is the scary code
    for(int i = 0, x = 1; i < binLength; i++, x+=2) { 
      if(binNumber[i] == '0') state = LOW;
      if(binNumber[i] == '1') state = HIGH;
...
void StopTheLights(){ //TODO: FIX THIS
  ...
  while(pushDown){ // I can not get out of this loop...
    delay(10);
  }
...
void AddCount(){// Function to increase count by one and update the lights
  ...
  /* get the length of the string */
  int binLength = binNumber.length(); 
  if(pressCounter <= 255) {  // if we have less or equal to 255 presses
                // here is the scary code
    for(int i = 0, x = 1; i < binLength; i++, x+=2) { 
      if(binNumber[i] == '0') state = LOW;
      if(binNumber[i] == '1') state = HIGH;
